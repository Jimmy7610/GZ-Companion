...
### M6 — Settlement Companion — *Implementation + human gameplay QA complete - prepared for v0.1.0-alpha.6*
- **Scope**: A real **LIVE** GameZone settlement dashboard (name, level, level name, role, bonus, treasury, currently-online same-settlement players) layered over a local settlement reference/planner/calculator (Översikt, Progression, Material, Medlemmar) covering the full current Settlement Levels 1.0 progression (50 levels, 49 upgrades, max level Imperium) and settlement foundation facts, all sourced from the official GameZone Wiki. Live facts come only from the same vanilla-visible TAB header text Home/Online already parse - never a second parser, never a new network source. A trusted live level automatically drives Progression/Material planning without ever overwriting the manually saved offline current level; every offline/local feature keeps working unchanged when disconnected or when no settlement is safely recognized.
- **Definition of Done**: Interactive Settlement tab loaded from `settlements.json` and `settlement-levels.json`; a level/target planner aggregating Coins and materials between two chosen levels; an optional, clearly-labeled last-known Chest Manager estimate; a local (not live-server) member organizer; a shared `GameZoneLiveContextBuilder` refresh path so Settlement never needs Home/Online opened first; structural fair-play tests proving no new network source, entity/world/chunk scanning, or automated command/chat. Confirmed via human gameplay QA against a live GameZone server - live identity/level/role/bonus/treasury match vanilla TAB, live level changes are picked up correctly, and Översikt/Progression/Material/Medlemmar all scroll and render correctly in both normal and compact layouts. GitHub release publication for `v0.1.0-alpha.6` is a separate, not-yet-done step. See [Settlement Companion](SETTLEMENT-COMPANION.md).
...
### M11 — Bounty Board — *Completed, released in v0.1.0-alpha.5*
